...
use axum::Json;
use mq_backend_lib::modules::{auth, learning, media, quran, users};
...
async fn main() {
    ...
    let app = axum::Router::new()
        ...
        .nest("/api/v1", auth::routes().merge(users::routes()).merge(media::routes()).merge(quran::routes()).merge(learning::routes()))
        .fallback(not_found)
        .with_state(state);
