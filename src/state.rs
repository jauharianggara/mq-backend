...
impl AppState {
    pub fn new(pool: MySqlPool, cfg: &AppConfig) -> Self {
        let jwt_secret = cfg
            .jwt_secret
            .clone()
            .unwrap_or_else(|| {
        ...
        Self {
            ...
            dev_expose_tokens: std::env::var("MQ_DEV_EXPOSE_TOKENS").map(|v| v == "true").unwrap_or(false),
            resend_gate_cache: Cache::builder().time_to_live(std::time::Duration::from_secs(60)).build(),
        }
    }
